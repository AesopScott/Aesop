...
**Generated:** 2026-05-11 14:05 UTC  
**Status:** 🔴 ISSUES FOUND  
...
- 🔴 **MISSING_DIR**: eval-benchmark → /ai-academy/modules/eval-benchmark/ does not exist
...
ℹ️ **Registry-driven mode detected.** `electives-hub.html` loads `course-registry.json` at runtime and renders only courses with `status: "live"`. No static `BASE_COURSES` array is present, so Checks H-1 and H-2 are not applicable in their literal form — the hub's course list is, by construction, the registry's live set.
...
1. MISSING_DIR: eval-benchmark → /ai-academy/modules/eval-benchmark/ does not exist

Additionally, **27 warning(s)** were recorded.

### Stats
- Registry courses: 131 (128 live, 3 coming soon)
- Retired courses in registry: 2
- courses.html internal links checked: 20
- courses.html `?course=` references: 123
- Electives hub mode: registry-driven (effective course set = 126 live)
- Module files verified: 780

### Notes
- electives-hub.html is registry-driven (fetches `course-registry.json` and renders `status: "live"` courses). No static `BASE_COURSES` array exists; Checks H-1, H-2, X-2, X-3 are not applicable in their literal form. Equivalent semantic checks were performed against the registry's live set.
