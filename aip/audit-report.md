...
**Generated:** 2026-05-27 14:10 UTC
**Status:** 🔴 ISSUES FOUND
...
> **Note:** `electives-hub.html` is v1.1.0 *registry-only* — it builds its course list at runtime from `course-registry.json` (`status === "live"`) and contains no hardcoded `BASE_COURSES`. Checks H-1, H-2, X-2 and X-3 therefore have no static entries to evaluate; every live registry course is rendered by construction.

## Cross-References

### Warnings (5)
- 🟡 **NOT_IN_COURSES_HTML**: registry course "society" (status: retired) has no `?course=` link from courses.html
- 🟡 **NOT_IN_COURSES_HTML**: registry course "ar-11" (status: retired) has no `?course=` link from courses.html
- 🟡 **NOT_IN_COURSES_HTML**: registry course "ar-8" (status: live) has no `?course=` link from courses.html
- 🟡 **NOT_IN_COURSES_HTML**: registry course "ap-7" (status: live) has no `?course=` link from courses.html
- 🟡 **NOT_IN_COURSES_HTML**: registry course "eval-benchmark" (status: live) has no `?course=` link from courses.html
...
1. Registry course `eval-benchmark` references `/ai-academy/modules/eval-benchmark/` which does not exist or is empty

**29 warning(s) noted** (non-blocking): 24 EXTRA_MODULES, 5 cross-reference.
...
- Electives hub BASE_COURSES: 0 (registry-only architecture)
