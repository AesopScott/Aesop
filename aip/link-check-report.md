...
**Generated:** 2026-07-26 10:09 UTC
**Status:** 🟡 WARNINGS — live fetch blocked at sandbox egress (7th week running); local proxy check still flags the same 1 candidate broken module URL
...
The **local repo proxy check** (see "Local proxy check" further down) again flagged the same URL it flagged the previous six weeks as a strong candidate for a live 404 — its backing file is still missing from the deployed source tree:

- `https://aesopacademy.org/ai-academy/modules/eval-benchmark/eval-benchmark-m1.html` — registry lists `eval-benchmark` as `status: "live"` with `modCount: 1`, but the directory `ai-academy/modules/eval-benchmark/` still does not exist in the repo. The course tile likely links to a page that was never built. No change vs. 2026-07-19 / 2026-07-05 / 2026-06-28 / 2026-06-21 / 2026-06-14 — the underlying fix has not landed. The daily `aip/audit-report.md` (latest 2026-07-25 14:12 UTC) has independently been flagging the same `MISSING_DIR` error every day this week, so the finding is corroborated by a second signal.

## Other Errors (5xx / Timeout / SSL)

All 911 URLs in the check list returned **fetch errors** this run, identical signature to the previous six weeks. Both `curl` and `web_fetch` from the routine container fail at the egress proxy with `HTTP/1.1 403 Forbidden` against `aesopacademy.org:443` — the proxy's own status endpoint (`$HTTPS_PROXY/__agentproxy/status`) reports `connect_rejected · "gateway answered 403 to CONNECT (policy denial or upstream failure)"` for the host. The block is at the network policy layer, not at the live origin. Today's control probes:

| Host | Status |
| --- | --- |
| `https://aesopacademy.org/` | 403 `host_not_allowed` (proxy CONNECT rejected; `curl` returns `000`, `WebFetch` returns "HTTP 403 Forbidden") |

The proxy's `noProxy` list still only exempts anthropic.com, npmjs, jsr, pypi, crates, and go-proxy hosts — every non-GitHub upstream is denied. This is the **seventh consecutive weekly run** blocked the same way (see 2026-06-07, 2026-06-14, 2026-06-21, 2026-06-28, 2026-07-05, and 2026-07-19 reports). The site itself is deployed via **Cloudflare Pages** (see `.github/workflows/deploy.yml`), so the block is not the origin — it's the sandbox's egress allow-list. Recommended fix: switch this routine's environment to a more permissive network policy, or add `aesopacademy.org` (and ideally `discord.gg`) to the egress allow-list for the current one. Reference: https://code.claude.com/docs/en/claude-code-on-the-web (network policies / environments).
...
Attempted via local extraction of `<a href="…">` and `<link href="…">` from `index.html` and `ai-academy/courses.html` since the live homepage couldn't be fetched. **One** unique external host referenced from those two pages (unchanged vs. 2026-07-19):
...
0 broken internal link(s) confirmed live — **no live data this run** (7th week in a row). All 911 URLs returned fetch errors at the routine container's egress layer (HTTP 403 `host_not_allowed`) before reaching the origin. This is the routine's environment, not a site outage.

**Local proxy check** (run as a fallback) surfaced the same **1 likely broken course (2 URLs)** that has been pending since 2026-06-14: `eval-benchmark` (registry entry exists with `status: live`, directory does not). The daily course audit (`aip/audit-report.md`, latest 2026-07-25) independently flags the same `MISSING_DIR` error, and the same course is missing from `courses.html` — so the tile is likely orphaned in the registry rather than actively linked from the site. 125 course-directory URLs without local `index.html` remain flagged as systemic — confirmation requires live fetch.
...
### Change vs. last run (2026-07-19)
- URL count unchanged at 911 (registry: 126 live courses, 765 live modules — same as last six weeks)
- Network block unchanged for `aesopacademy.org` (still `host_not_allowed`; proxy `noProxy` allow-list unchanged)
- Candidate broken link unchanged — `eval-benchmark-m1.html` still missing; no fix landed in the past week; the daily audit report has been flagging the same `MISSING_DIR` continuously
...
- Site plumbing note carried forward — deploy target is Cloudflare Pages (`.github/workflows/deploy.yml`), which explains how directory URLs without local `index.html` can still serve 200 in production
