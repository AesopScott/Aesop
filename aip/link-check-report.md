...
**Generated:** 2026-08-02 10:10 UTC
...
| Host | Status |
| --- | --- |
| `https://aesopacademy.org/` | 403 `host_not_allowed` (proxy CONNECT rejected, `curl` exit 56) |
| `https://aesopacademy.org/ai-academy/courses.html` | 403 `host_not_allowed` (proxy CONNECT rejected, `curl` exit 56) |
| `https://aesopacademy.org/ai-academy/modules/eval-benchmark/eval-benchmark-m1.html` | 403 `host_not_allowed` (proxy CONNECT rejected, `curl` exit 56) |
...
Attempted via local extraction of `<a href="…">` attributes from `index.html` and `ai-academy/courses.html` since the live homepage couldn't be fetched. **One** unique external host referenced from those two pages (unchanged vs. 2026-07-19):
...
**Local proxy check** (run as a fallback) surfaced the same **1 likely broken course (2 URLs)** that has been pending since 2026-06-14: `eval-benchmark` (registry entry exists with `status: live`, directory does not). The daily course audit (`aip/audit-report.md`, latest 2026-08-01) independently flags the same `MISSING_DIR` error, and the same course is missing from `courses.html` — so the tile is likely orphaned in the registry rather than actively linked from the site. 125 course-directory URLs without local `index.html` remain flagged as systemic — confirmation requires live fetch.
...
- URL count unchanged at 911 (registry: 126 live courses, 765 live modules — same as prior six weeks)
- Network block unchanged for `aesopacademy.org` (still `host_not_allowed`)
- Candidate broken link unchanged — `eval-benchmark-m1.html` still missing; no fix landed in the past two weeks; the daily audit report continues to flag the same `MISSING_DIR`
