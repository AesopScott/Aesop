...
- `https://discord.gg/pKDa5ryX` — fetch error: HTTP 403 (egress-blocked, not site-blocked)
...
- **770** of 897 URL targets resolved to a file present in the repo.
- **126** course-directory URLs lack an `index.html` in the repo — see "Systemic course-directory pattern" below.
- **1** URL target has no backing path: `ai-academy/modules/eval-benchmark/eval-benchmark-m1.html` — already listed under 404 candidates above.

### Systemic course-directory pattern

All 126 live course-directory URLs (e.g. `/ai-academy/modules/ai-and-creativity/`) map to a directory that exists but does not contain an `index.html` — this is systemic across the whole live catalog, not a per-course bug. Live behavior of these URLs cannot be determined from the repo alone; on Cloudflare Pages these typically resolve via directory-index conventions or a rewrite rule, so many may serve 200 in production. Once the egress block is resolved, the next run should clarify by fetching these URLs.

### Registry change vs. previous run

- **Total URL count fell from 911 → 897** (−14). Driver: the registry's `_meta` block (and therefore `_meta.languages`) is no longer present, so the routine built 0 language-variant `courses.html` URLs this week vs. 14 in prior runs. Live-course count is unchanged at 126. If language pages are still deployed but the registry no longer advertises them, they will be silently skipped by this crawler until `_meta.languages` is restored — worth confirming on the next live-fetch-capable run.

---

## Summary

0 broken internal link(s) confirmed live — **no live data this run** (8th week in a row). All 897 URLs returned fetch errors at the routine container's egress layer (HTTP 403 `host_not_allowed`) before reaching the origin. This is the routine's environment, not a site outage.

**Local proxy check** (run as a fallback) surfaced the same **1 likely broken course (1 URL)** that has been pending since 2026-06-14: `eval-benchmark` (registry entry exists with `status: live`, directory does not). The daily course audit (`aip/audit-report.md`, latest 2026-08-22) independently flags the same `MISSING_DIR` error, and the same course is missing from `courses.html` — so the tile is likely orphaned in the registry rather than actively linked from the site. 126 course-directory URLs without local `index.html` remain flagged as systemic — confirmation requires live fetch.
...
### Stats
- Internal URLs built from seeds + `course-registry.json`: 897
- External URLs spot-checked (local extraction): 1
- Live URLs successfully fetched: 0 (all blocked at egress)
- Run duration: ~1 minute (short — no live fetch attempted after control probes failed)
