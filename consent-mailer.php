...
define('SMTP_USER',     'a78a3c001@smtp-brevo.com');

$_smtpPass = defined('AESOP_CONSENT_SMTP_PASS') ? AESOP_CONSENT_SMTP_PASS : '';
if (!$_smtpPass) {
    error_log('consent-mailer: AESOP_CONSENT_SMTP_PASS is not set');
    http_response_code(500);
    echo json_encode(['ok' => false, 'error' => 'Mail configuration error']);
    exit;
}
...
function sendAdminNotification($record) {
    if (!defined('SMTP_PASS') || !SMTP_PASS) {
        error_log('consent-mailer: skipping admin notification — SMTP_PASS not set');
        return;
    }
    $mail = new PHPMailer(true);
    try {
