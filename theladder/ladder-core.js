...
// ladder-core.js — page-agnostic Ladder engine extracted from ladder-app.js
// (BUILD-LADDER-REDESIGN.md §7). State, persistence, Firebase auth, proxy
// calls, placement/certification orchestration, i18n helpers, and guarded
// render functions shared by the page entry modules (hub-page.js,
// assessment-page.js, climb-page.js, transcript-page.js). DOM lookups are
// bound in bindDom() at init and every use is null-guarded, so any page can
...
// Bound at init time so this module stays DOM-light at the top level (BUILD §7).
// Pages only contain their own subset of these ids; missing lookups stay null
// and every consumer is guarded.
function bindDom() {
  Object.assign(el, {
...
function showCertificationSuccess(record) {
  // Create and show celebration modal (skinned with brand tokens)
  const modal = document.createElement('div');
  modal.id = 'certificationSuccessModal';
  modal.className = 'cert-success-overlay';
  ...
  modal.innerHTML = `
    <div class="cert-success-card">
      ...
      <p>${escapeHtml(record.title)}<br><small>Independent validation confirmed</small></p>
      <div class="cert-success-share">
        <p>Share your achievement:</p>
        <textarea id="certificationMessage" readonly>${escapeHtml(message)}</textarea>
        <button type="button" data-copy-certification>Copy to clipboard</button>
      </div>
      <button type="button" class="secondary" data-close-certification>View your certification</button>
    </div>
  `;

  modal.querySelector('[data-copy-certification]').addEventListener('click', (event) => {
    const messageBox = modal.querySelector('#certificationMessage');
    messageBox.select();
    document.execCommand('copy');
    event.currentTarget.textContent = 'Copied!';
    setTimeout(() => {
      const button = modal.querySelector('[data-copy-certification]');
      if (button) button.textContent = 'Copy to clipboard';
    }, 2000);
  });
  modal.querySelector('[data-close-certification]').addEventListener('click', () => modal.remove());
...
async function openTopicFromSearch(topicId, assignToPath) {
  ...
  await persist();
  // The rung workspace lives on climb.html now — navigate there when this
  // page has no workspace column (e.g. search from the hub).
  if (!document.querySelector('.topic-column')) {
    window.location.href = climbUrlFor(topic);
    return;
  }
  render();
...
function renderCertificationActions() {
  ...
  if (!recentCert.celebrationShown) {
    const transcript = document.getElementById('transcript');
    if (!transcript || document.getElementById('certificationCelebrationOffer')) return;
    const actionDiv = document.createElement('div');
    actionDiv.id = 'certificationCelebrationOffer';
    actionDiv.className = 'panel panel-accent panel-pad';
    actionDiv.style.textAlign = 'center';
    actionDiv.style.marginBottom = '14px';
    actionDiv.innerHTML = `
      <p style="margin: 0 0 12px; font-weight: 600;">${escapeHtml(recentCert.title)}</p>
      <button type="button" data-show-recent-certification>Show Celebration &amp; Share</button>
    `;
    actionDiv.querySelector('[data-show-recent-certification]').addEventListener('click', showRecentCertification);
...
function renderProgress() {
  ...
  if (el.progressText) el.progressText.textContent = `${count} of ${total} rungs completed`;
  if (el.tierCompletionStatus) el.tierCompletionStatus.textContent = `${completedTierCount()} / ${LADDER_TIERS.length} tiers completed`;
  renderHeroProgress(count, total);
}

// The old tier-rail renderer (renderTiers) moved to hub-page.js as the tier
// accordion; this module only computes the per-tier progress it needs.
function tierProgress(tier) {
  ...
  return {
    ...
    total: tier.topics.length,
    placedOut: granted.has(tier.id),
    assignedCount: tier.topics.filter((topic) => assigned.has(topic.id)).length,
    complete: granted.has(tier.id) || done >= tier.topics.length
  };
}

function renderTopicPicker(tier) {
  if (!document.querySelector('.topic-head')) return;
...
function renderVocabularyDefinition(tier) {
  if (!el.vocabDefinitionTerm || !el.vocabDefinitionBox) return;
  ...
  `;
  if (el.vocabPromptInput) el.vocabPromptInput.placeholder = `Ask about ${term}`;
}
...
function renderVocabulary(tier) {
  if (!el.vocabList || !el.vocabCount) return;
  if (!tier.vocabulary.includes(state.activeVocabTerm)) {
    state.activeVocabTerm = tier.vocabulary[0];
  }
...
function renderConversationMode() {
  ...
  // Exam-in-progress banner (F14) — visible while a certification exam runs
  const banner = document.getElementById('examInProgressBanner');
  if (banner) {
    banner.style.display = hasMessages && isCertification ? 'block' : 'none';
  }
...
function renderTopic() {
  ...
  if (el.activeTierLabel) el.activeTierLabel.textContent = `${tier.name} - ${topic.id}`;
  if (el.activeTopicTitle) el.activeTopicTitle.textContent = topic.title;
  if (el.completeTopicBtn) {
...
function renderEvaluationPanel() {
  ...
  if (el.activeEvaluationTarget) el.activeEvaluationTarget.textContent = targetText;
...
// Dark-mode toggling (old F2) is owned by theme.js, which mirrors the mode to
// the legacy 'aesop-theme' key for the rest of aesopacademy.org.

const pageRenderers = [];

function registerPageRenderer(renderer) {
  pageRenderers.push(renderer);
}
...
function render() {
  ...
  renderTopic();
  renderTranscript();
  pageRenderers.forEach((renderer) => {
    try {
      renderer();
    } catch (error) {
      console.warn('Page renderer failed:', error);
    }
  });
...
// Deep link target for a topic's workspace on climb.html (BUILD §1, §6.5).
function climbUrlFor(topic) {
  const tier = tierById(topic.tierId);
  const order = tier.topics.find((item) => item.id === topic.id)?.order || 1;
  return `/theladder/climb.html?tier=${tier.order}&rung=${order}`;
}

// Page-module helper: activate a tier/topic the same way the old tier rail
// and topic strip clicks did (reset any in-flight conversation, persist, render).
async function setActiveTopic(tierId, topicId, options = {}) {
  const tier = tierById(tierId);
  state.activeTierId = tier.id;
  const topic = tier.topics.find((item) => item.id === topicId) || tier.topics[0];
  state.activeTopicId = topic.id;
...
let coreInitStarted = false;

async function initCore(options = {}) {
  if (coreInitStarted) return;
  coreInitStarted = true;
  bindDom();
  loadLocal();
  options.afterLoad?.('local');
...
export {
  ...
  completedCount,
  completedTierCount,
  credentialTierCounts,
  buildCertificationTranscript,
  certificationTranscriptLines,
  normalizeLearnerId,
  statusLabel,
